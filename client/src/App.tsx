...
function LandingPage() {
  ...
  return (
    ...
    >
      <header className="absolute top-0 w-full z-30 px-4 md:px-6 py-3 md:py-4 flex items-center justify-between border-b border-[var(--border-color)] bg-[var(--bg-primary)]/80 backdrop-blur-md">
        ...
        <div className="flex items-center space-x-2 md:space-x-4">
          <button onClick={toggleTheme} className="p-1.5 md:p-2 rounded-full hover:bg-[var(--glass-hover-bg)] transition-all cursor-pointer">
            {theme === 'dark' ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
          </button>
          <button
            onClick={() => navigate('/login')}
            className="base-btn px-3 py-1.5 md:px-4.5 md:py-2 rounded-lg text-[10px] md:text-xs font-mono tracking-wider cursor-pointer"
          >
            LOGIN
          </button>
          <button
            onClick={() => navigate('/repo/gothinkster/node-express-realworld-example-app')}
            className="px-3 py-1.5 md:px-4.5 md:py-2 bg-[var(--accent-primary)] hover:opacity-90 rounded-lg text-[10px] md:text-xs font-mono tracking-wider text-white transition-all cursor-pointer flex items-center"
          >
