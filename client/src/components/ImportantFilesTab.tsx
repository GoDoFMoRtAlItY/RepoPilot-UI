...
export default function ImportantFilesTab() {
  ...
  const getFileIcon = (fileName: string) => {
    ...
    switch (ext) {
      case 'ts':
      case 'tsx':
        return <FileCode className="w-5 h-5 text-blue-400 shrink-0 animate-pulse" />
      case 'js':
      case 'jsx':
  ...
  const FolderNode = ({ node, level = 0 }: { node: any, level?: number }) => {
    ...
    return (
      <div className={`space-y-3.5 ${level > 0 ? 'ml-6 border-l border-slate-800/50 pl-4' : ''}`}>
        <div 
          className="flex items-center space-x-2 text-slate-400 border-b border-slate-900 pb-2 cursor-pointer hover:text-cyan-400 transition-colors"
          onClick={() => setIsOpen(!isOpen)}
        >
          <FolderOpen className="w-4.5 h-4.5 text-cyan-400/80" />
                    ...
                    {folderFiles.map((child: any) => {
                      ...
                      return (
                        <div
                          ...
                          className={`glass-panel p-4.5 rounded-xl transition-all duration-300 cursor-pointer flex flex-col justify-between border-slate-850 hover:border-slate-700 bg-slate-950/20 hover:bg-slate-900/10 hover:shadow-[0_0_15px_rgba(6,182,212,0.02)] ${
                          ...
                          <div className="space-y-2.5">
                            <div className="flex items-center justify-between gap-3">
                              <div className="flex items-center space-x-2.5 min-w-0">
                                ...
                                <span className="text-white text-xs font-bold truncate tracking-wide">{file.fileName}</span>
                              </div>
                              <div className="flex items-center space-x-1.5 shrink-0">
                                <span className="text-[9px] text-slate-500 uppercase font-mono tracking-wider bg-slate-900 border border-slate-850 px-1.5 py-0.5 rounded">
                            ...
                            <div className="text-xs text-slate-400 font-sans leading-relaxed min-h-[32px]">
                              {file.loading ? (
                                <div className="space-y-1.5 animate-pulse">
                                  <div className="h-3 bg-slate-800/80 rounded w-full" />
                                  <div className="h-3 bg-slate-800/80 rounded w-2/3 mt-2" />
                                  <span className="text-cyan-500/70 text-[10px] mt-1 block">Generating AI explanation...</span>
                                </div>
                              ) : (
                          ...
                          <AnimatePresence>
                            {isExpanded && (
                              <motion.div
                                ...
                                animate={{ height: 'auto', opacity: 1 }}
                                exit={{ height: 0, opacity: 0 }}
                                className="overflow-hidden mt-3.5 pt-3.5 border-t border-slate-900 font-sans text-xs text-slate-400"
                              >
                                <div className="space-y-2.5">
                                  <div className="flex flex-col gap-2">
                                    <a
                                      ...
                                      className="w-full py-2 bg-slate-900 hover:bg-slate-850 text-center border border-slate-800 hover:border-cyan-500/30 text-[10px] font-bold text-slate-300 hover:text-white rounded transition-all block tracking-wider"
                                    >
        ...
        >
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-800 hover:border-slate-700 focus:border-cyan-400 rounded-lg pl-11 pr-4 py-3 text-xs text-slate-200 placeholder-slate-500 transition-all font-mono outline-none"
            />
          </div>
          <button
            type="submit"
            disabled={isTreeLoading}
            className="px-6 py-3 bg-cyan-600 hover:bg-cyan-500 disabled:bg-slate-800 disabled:text-slate-500 text-white rounded-lg text-xs font-bold tracking-wider transition-all flex items-center justify-center space-x-2 cursor-pointer shadow-[0_0_12px_rgba(34,211,238,0.15)] disabled:shadow-none"
          >
            {isTreeLoading ? (
              <>
      ...
      {isTreeLoading && (
        <div className="glass-panel p-12 text-center rounded-xl flex flex-col items-center justify-center space-y-4">
          ...
          <div className="space-y-1 font-sans">
            <h4 className="text-white text-sm font-semibold">Fetching remote file tree...</h4>
            <p className="text-slate-500 text-xs">Accessing GitHub API. Please wait.</p>
          </div>
        </div>
      )}
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center justify-between">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 hover:border-slate-700 focus:border-cyan-400 rounded-lg pl-10 pr-4 py-2 text-xs text-slate-200 placeholder-slate-500 transition-all font-mono outline-none"
              />
            </div>



            <div className="flex items-center space-x-2 text-xs text-slate-500 bg-slate-950/20 border border-slate-800/50 px-3.5 py-2 rounded-lg">
                <GitBranch className="w-3.5 h-3.5 text-purple-400" />
                <span>Branch: <strong className="text-slate-300">{defaultBranch}</strong></span>
                <span className="text-slate-700">|</span>
                <span>SHA: <code className="text-slate-400 text-[10px]">{commitSha.substring(0, 7)}</code></span>
              </div>
          </div>
