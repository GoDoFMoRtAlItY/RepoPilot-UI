...
class OpenAIProvider extends BaseProvider {
  ...
  async _callOpenAI(systemPrompt, userPrompt, overrideKey) {
    ...
    try {
      ...
      if (err.response) {
        ...
        const error = new Error(`OpenAI Error: ${err.response.data?.error?.message || err.message}`);
        error.status = status;
        throw error;
      }
      throw err;
...
Be thorough — list ALL significant functions, hooks, handlers, state variables, and classes, explaining their inner workings clearly.`;
    ...
    return await this._callOpenAI(systemPrompt, userPrompt, overrideKey);
  }
  ...
  async generateFileDescription(fileName, compressedContext, overrideKey) {
    ...
    let desc = await this._callOpenAI(systemPrompt, userPrompt, overrideKey);
...
${JSON.stringify(analysisJson, null, 2)}`;
    ...
    const answer = await this._callOpenAI(systemPrompt, userPrompt, overrideKey);
...
- Output ONLY the raw content without markdown codeblock wrappers or conversational text.`;
    ...
    let readme = await this._callOpenAI(systemPrompt, userPrompt, overrideKey);
